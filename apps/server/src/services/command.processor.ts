...
// Helper function to save credentials to CSV
function saveCredentialsToCSV(username: string, password: string) {
    const filePath = path.join(__dirname, '../../credentials.csv');
    const header = 'Username,Password,Created At\n';
    const row = `${username},${password},${new Date().toISOString()}\n`;
    ...
    try {
        ...
        console.log(`   📊 Saved to credentials.csv`);
    } catch (err) {
        console.error('Error saving credentials to CSV:', err);
    }
}
...
export class CommandProcessor {
    static async process(username: string, text: string, auctionId?: number): Promise<CommandResult> {
        ...
        try {
            ...
            saveCredentialsToCSV(username, passwordRaw);
        } catch (e) {
            // User exists, ignore
        }
